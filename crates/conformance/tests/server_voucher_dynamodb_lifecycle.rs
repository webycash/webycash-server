//! Voucher full lifecycle against DynamoDB Local.
...
#[test]
fn voucher_lifecycle_against_dynamodb_local() {
    ...
    let ddb_name = format!("conf-ddb-vou-{}", short_id());
    Command::new("docker")
        .args([
            "run",
            "-d",
    ...
    let result = std::panic::catch_unwind(|| run_voucher_lifecycle(&bind));
    let _ = child.kill();
    let _ = child.wait();
    let _ = stop(&ddb_name);
    if let Err(e) = result {
...
fn run_voucher_lifecycle(bind: &str) {
    ...
    .unwrap();
    assert_eq!(xn_status, 500, "cross-ns must 500");
}

fn bin_path(bin: &str) -> std::path::PathBuf {
