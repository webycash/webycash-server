...
//!
//! Boots `Server<Voucher, S>` where `S` is selected at runtime from
//! `WEBCASH_DB_BACKEND` ∈ {redis, dynamodb, fdb (feature-gated),
//! redis_fdb (feature-gated)}. Vouchers are always-splittable bearer
//! credits, issuer-namespaced by `(contract_id, issuer_fp)`.
...
use webycash_server::asset_voucher::Voucher;
use webycash_server::auth::IssuerRegistry;
use webycash_server::mining::{MiningConfig, MiningMode};
use webycash_server::server_core::{serve_issued, ServeConfig, Server};
...
use webycash_server::storage::NamespacedKeys;
...
async fn main() -> anyhow::Result<()> {
    ...
    // Optional: load OpenPGP V4 armored certs from a file. One armored
    // cert per `-----BEGIN PGP PUBLIC KEY BLOCK-----` block; primary key
    // must be Ed25519. Fingerprint is discovered from the cert.
    ...
    match db_backend.as_str() {
        ...
        "fdb" => {
            ...
            let store = FdbStore::<Voucher, _>::new(cluster_file.as_deref(), NamespacedKeys)
